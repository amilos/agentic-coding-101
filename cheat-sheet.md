...
**Good:** `TransferService.Transfer accepts a 0 or negative amount, letting money flow the wrong way. Reject non-positive amounts, add an xUnit test, and show me the diff before applying.`

**Give context:** name the file/class, the rule, the expected behaviour, and the constraint (e.g. "money is `decimal`").

**Ask for a plan/diff first:** end prompts with *"Propose a plan"* or *"Show me the diff before applying"* — review, then approve.
...
| Request diff before apply | Auto-apply and hope |
...
| **Appium + WinAppDriver** | Win32 VCL UI automation |
...
- **Plan:** `Using the requirements-to-tasks skill, read ISSUES.md issue #1 and produce a structured task list with acceptance criteria and the files likely to change.`
...
- **Implement:** `Implement ISSUES.md issue #1 (daily transfer limit) across PaymentService. Update TransferService and add xUnit tests. Show me the diff before applying.`
- **Atlas migration:** `Add a nullable Reference column to LedgerEntries using Atlas declarative schema (atlas/schema.hcl) and generate the migration.`
...
- **Appium:** `Scaffold an Appium + WinAppDriver test that loads a statement for account 1001 in StatementViewer and asserts the list is populated.`
